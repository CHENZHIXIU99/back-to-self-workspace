...
import {
  ...
  CupSoda, Flower2, NotebookPen, Pause, Pencil, Play, Plus, Search, Settings, Shirt, Sparkles, Trash2,
...
type TimerState={running:boolean;end:number;remaining:number;plannedMinutes:number};
  ...
  const [timer, setTimer] = useState<TimerState>(() => {
    if (typeof window !== "undefined") {
      try {
        ...
        if(saved){const parsed=JSON.parse(saved);return {...parsed,plannedMinutes:parsed.plannedMinutes||25}}
      } catch {}
    }
    return {running:false,end:0,remaining:25*60,plannedMinutes:25};
  });
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if(!remaining){
        ...
        update(d=>({...d,focusSessions:d.focusSessions+1,focusRecords:[...d.focusRecords,{id:crypto.randomUUID(),date:localDateKey(),minutes:timer.plannedMinutes,completedAt:new Date().toISOString()}]}));
        notify("本轮专注已完成并记录");
      }else setTimer(t=>({...t,remaining}));
    },1000);
    return () => clearInterval(id);
  }, [timer.running,timer.end,timer.plannedMinutes]);
  ...
  return <div className={`app${theme==="dark"?" dark":""} font-${font}`}>
    ...
    <main>
      ...
      <div className="content">
        ...
        page==="tasks" ? <Tasks data={data} toggle={toggleTask} open={setModal} edit={item=>{setEditingItem({kind:"task",value:item});setModal("task")}} remove={remove}/> :
    ...
    {modal&&<Editor modal={modal} close={()=>{setModal(null);setEditingItem(null)}} update={update} notify={notify} timer={timer} setTimer={setTimer} workspaceName={workspaceName} defaultScheduleDate={scheduleDate} workspaceData={data} editingItem={editingItem}/>}
...
function Health({data,update,notify}:{data:WorkspaceData;update:(f:(d:WorkspaceData)=>WorkspaceData)=>void;notify:(message:string)=>void}){
  ...
  {healthModal&&<Editor modal={healthModal} close={closeHealthModal} update={update} notify={notify} timer={{running:false,end:0,remaining:1500,plannedMinutes:25}} setTimer={()=>{}} workspaceName={data.profile.name.trim()?`${data.profile.name.trim()}的工作台`:"我的工作台"} workspaceData={data} editingPeriod={editingPeriod} editingItem={editingDrink}/>}
  </>}
...
function Editor({modal,close,update,notify,timer,setTimer,workspaceName="我的工作台",defaultScheduleDate=localDateKey(),workspaceData,editingPeriod=null,editingItem=null}:{modal:Exclude<Modal,null>;close:()=>void;update:(f:(d:WorkspaceData)=>WorkspaceData)=>void;notify:(x:string)=>void;timer:TimerState;setTimer:React.Dispatch<React.SetStateAction<TimerState>>;workspaceName?:string;defaultScheduleDate?:string;workspaceData:WorkspaceData;editingPeriod?:WorkspaceData["periods"][number]|null;editingItem?:EditingItem|null}){
  const today=localDateKey();
  const [form,setForm]=useState<Record<string,string>>(()=>{
    if(modal==="schedule")return {date:defaultScheduleDate,time:"09:00",type:"个人安排"};
    if(editingPeriod)return {...editingPeriod};
    if(editingItem?.kind==="task"){const item=editingItem.value;return {title:item.title,next:item.next,standard:item.standard,minutes:String(item.minutes),category:item.category,priority:item.priority}}
  ...
  function save(){
    if(modal==="task"){if(!form.title)return notify("请填写任务名称");const minutes=Math.min(600,Math.max(5,+form.minutes||30));const next={title:form.title,next:form.next||"明确下一步并开始",standard:form.standard||"达到预期结果",minutes,category:form.category||"AI训练",priority:form.priority||"重要不紧急"};update(d=>({...d,tasks:editingItem?.kind==="task"?d.tasks.map(item=>item.id===editingItem.value.id?{...item,...next}:item):[...d.tasks,{id:crypto.randomUUID(),...next,done:false,createdDate:today,completedDate:""}]}));}
  ...
  function finishFocus(){
    const elapsedSeconds=timer.plannedMinutes*60-timer.remaining;
    if(elapsedSeconds<60)return notify("专注满 1 分钟后再记录");
    const minutes=Math.max(1,Math.round(elapsedSeconds/60));
    setTimer({running:false,end:0,remaining:25*60,plannedMinutes:25});
  ...
  if(modal==="focus"){const mm=String(Math.floor(timer.remaining/60)).padStart(2,"0"),ss=String(timer.remaining%60).padStart(2,"0");return <div className="modal-backdrop"><div className="modal focus-modal"><button className="modal-close" onClick={close}><X/></button><span className="eyebrow">专注当下</span><h2>本轮专注</h2><div className="timer-display">{mm}:{ss}</div><p>先完成手上的这一小步。</p><div className="timer-presets">{[15,25,45].map(v=><button onClick={()=>setTimer({running:false,end:0,remaining:v*60,plannedMinutes:v})} key={v}>{v} 分钟</button>)}</div><div className="focus-actions"><button className="primary large" onClick={()=>setTimer(t=>t.running?{...t,running:false}:{...t,running:true,end:Date.now()+t.remaining*1000})}>{timer.running?<><Pause/>暂停</>:<><Play/>开始</>}</button><button className="secondary" onClick={finishFocus}>结束并记录</button></div></div></div>}
  ...
  return <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&close()}><div className="modal form-modal"><button className="modal-close" onClick={close} aria-label="关闭"><X/></button><span className="eyebrow">{workspaceName}</span><h2>{titles[modal as keyof typeof titles]}</h2>
    {modal==="task"&&<><Field label="任务名称"><input autoFocus value={form.title||""} onChange={e=>set("title",e.target.value)} placeholder="例如：完成一轮 AI 训练测试"/></Field><div className="form-grid"><Field label="类型"><select value={form.category||"AI训练"} onChange={e=>set("category",e.target.value)}><option>AI训练</option><option>提示词测试</option><option>素材整理</option><option>学习研究</option><option>生活事务</option><option>其他</option></select></Field><Field label="优先级"><select value={form.priority||"重要不紧急"} onChange={e=>set("priority",e.target.value)}><option>重要不紧急</option><option>紧急重要</option><option>普通</option><option>可延后</option></select></Field></div><Field label="预计时长（分钟）"><input type="number" min="5" value={form.minutes||"30"} onChange={e=>set("minutes",e.target.value)}/></Field><Field label="具体下一步"><textarea value={form.next||""} onChange={e=>set("next",e.target.value)} placeholder="15–30 分钟内能完成什么？"/></Field><Field label="完成标准"><textarea value={form.standard||""} onChange={e=>set("standard",e.target.value)} placeholder="怎样算完成？"/></Field></>}
