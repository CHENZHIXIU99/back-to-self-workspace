...
export default function HomePage() {
  ...
  async function importFile(file:File){ try{ const parsed=JSON.parse(await file.text()); if(!parsed.schemaVersion||!Array.isArray(parsed.tasks)) throw new Error(); const restoredSchedule=(parsed.schedule||[]).map((x:Partial<WorkspaceData["schedule"][number]>,index:number)=>({id:x.id||`imported-schedule-${index}`,date:x.date||localDateKey(),time:x.time||"09:00",title:x.title||"未命名日程",type:x.type||"个人安排"})); setData({...seedData,...parsed,schemaVersion:7,updatedAt:new Date().toISOString(),profile:{...seedData.profile,...parsed.profile},health:{...seedData.health,...parsed.health},drinks:(parsed.drinks||[]).map((x:WorkspaceData["drinks"][number])=>({...x,sticker:x.sticker||false})),outfits:parsed.outfits||[],periods:parsed.periods||[],schedule:restoredSchedule});notify("数据已恢复"); }catch{notify("无法导入：文件格式不正确");} }
  ...
  return <div className={`app${theme==="dark"?" dark":""} font-${font}`}>
    ...
    {modal&&<Editor modal={modal} close={()=>setModal(null)} update={update} notify={notify} timer={timer} setTimer={setTimer} workspaceName={workspaceName} defaultScheduleDate={scheduleDate}/>}
...
function Health({data,update,notify}:{data:WorkspaceData;update:(f:(d:WorkspaceData)=>WorkspaceData)=>void;notify:(message:string)=>void}){
  ...
  const updateTodayHealth=(patch:Partial<WorkspaceData["health"]>)=>update(d=>({...d,health:{...(d.health.date===today?d.health:{date:today,sleep:null,water:0,meals:null}),...patch,date:today}}));
  return <><section className="two-col health-columns">
    <div className="panel"><div className="panel-head"><div><span className="eyebrow">身体状态</span><h3>今天的基础记录</h3></div></div><div className="record-stack">
      ...
      <div><span className="field-label">今日饮水量：{todayHealth.water} ml</span><div className="seg water-actions">{[200,300,500].map(v=><button onClick={()=>update(d=>{const current=d.health.date===today?d.health:{date:today,sleep:null,water:0,meals:null};return {...d,health:{...current,water:current.water+v,date:today}}})} key={v}><Droplets size={16}/>+{v}ml</button>)}</div></div>
  ...
  <section className="panel period-panel"><div className="panel-head"><div><span className="eyebrow">女性健康</span><h2>姨妈周期</h2><p>只记录自己的日期和感受，不做医疗诊断。</p></div><button className="primary period-button" onClick={()=>setHealthModal("period")}><Flower2 size={17}/>记录本次姨妈</button></div>
    {data.periods.length?<div className="period-history">{[...data.periods].reverse().map(x=><div className="period-row" key={x.id}><div className="period-day"><Flower2/><b>{x.startDate.slice(5)}</b></div><div><b>{x.startDate} 至 {x.endDate||"进行中"}</b><p>经量：{x.flow||"未记录"}{x.note?` · ${x.note}`:""}</p></div></div>)}</div>:<Empty title="还没有周期记录" note="点击上方“记录本次姨妈”，保存开始日期、结束日期和感受。"/>}
  </section>
...
function Finance({data,income,expense,balance,monthKey,open,remove}:{data:WorkspaceData;income:number;expense:number;balance:number;monthKey:string;open:(m:Modal)=>void;remove:(k:"transactions",id:string)=>void}){
...
function Editor({modal,close,update,notify,timer,setTimer,workspaceName="我的工作台",defaultScheduleDate=localDateKey()}:{modal:Exclude<Modal,null>;close:()=>void;update:(f:(d:WorkspaceData)=>WorkspaceData)=>void;notify:(x:string)=>void;timer:{running:boolean;end:number;remaining:number};setTimer:React.Dispatch<React.SetStateAction<{running:boolean;end:number;remaining:number}>>;workspaceName?:string;defaultScheduleDate?:string}){
  const [form,setForm]=useState<Record<string,string>>(()=>modal==="schedule"?{date:defaultScheduleDate,time:"09:00",type:"个人安排"}:{});
  ...
  const titles:{[K in Exclude<Modal,null|"focus"|"emotion">]:string}={task:"添加今日任务",schedule:"添加日程",expense:"快速记一笔",temporary:"添加临时任务",weight:"记录体重",drink:"记录每日饮品",outfit:"记录今日穿搭",period:"记录姨妈周期",note:"添加学习笔记"};
