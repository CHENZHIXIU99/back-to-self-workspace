...
export type WorkspaceData={
  ...
  health:{sleep:number|null;water:number;meals:number|null};weights:{id:string;value:number;date:string}[];
  drinks:{id:string;name:string;type:string;amount:number|null;photo:string;date:string;time:string}[];
...
export const seedData:WorkspaceData={schemaVersion:3,createdAt:now,updatedAt:now,status:"",focusSessions:0,
tasks:[],temporary:[],transactions:[],health:{sleep:null,water:0,meals:null},weights:[],drinks:[],outfits:[],periods:[],notes:[],schedule:[],boundaries:[],review:{best:"",next:""}};

const DB="back-to-self",STORE="workspace",KEY="primary";
export async function loadWorkspace():Promise<WorkspaceData>{try{const db=await openDB(DB,1,{upgrade(db){if(!db.objectStoreNames.contains(STORE))db.createObjectStore(STORE)}});const saved=await db.get(STORE,KEY) as WorkspaceData|undefined;if(!saved)return seedData;if((saved.schemaVersion||1)<2){const demoIds=new Set(["t1","t2","t3","tmp1","m1","m2","m3","w1","w2","w3","w4","n1"]);return {...saved,schemaVersion:3,status:"",focusSessions:0,tasks:saved.tasks.filter(x=>!demoIds.has(x.id)),temporary:saved.temporary.filter(x=>!demoIds.has(x.id)),transactions:saved.transactions.filter(x=>!demoIds.has(x.id)),weights:saved.weights.filter(x=>!demoIds.has(x.id)),notes:saved.notes.filter(x=>!demoIds.has(x.id)),schedule:[],health:{sleep:null,water:0,meals:null},drinks:[],outfits:[],periods:[],boundaries:[],review:{best:"",next:""}}}return {...seedData,...saved,schemaVersion:3,drinks:saved.drinks||[],outfits:saved.outfits||[],periods:saved.periods||[]}}catch{return seedData}}
