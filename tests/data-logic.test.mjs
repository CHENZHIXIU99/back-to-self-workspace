...
test("倒计时按绝对结束时间恢复，过期后只显示完成",()=>{
  const timer={mode:"countdown",running:true,endAt:1_060_000,startedAt:0,remainingSeconds:120,elapsedSeconds:0,durationSeconds:120};
  assert.deepEqual(focusTimerSnapshot(timer,1_030_000),{displaySeconds:30,elapsedSeconds:90,completed:false});
  assert.deepEqual(focusTimerSnapshot(timer,1_061_000),{displaySeconds:0,elapsedSeconds:120,completed:true});
});
test("正计时按开始时间恢复，暂停后保持累计秒数",()=>{
  const running={mode:"stopwatch",running:true,endAt:0,startedAt:1_000_000,remainingSeconds:0,elapsedSeconds:10,durationSeconds:0};
  assert.deepEqual(focusTimerSnapshot(running,1_075_900),{displaySeconds:75,elapsedSeconds:75,completed:false});
  assert.deepEqual(focusTimerSnapshot({...running,running:false,startedAt:0,elapsedSeconds:75},9_000_000),{displaySeconds:75,elapsedSeconds:75,completed:false});
});
test("自定义专注分钟限制在合理范围，记录按实际秒数换算",()=>{
  assert.equal(normalizeFocusMinutes("",25),1);
  assert.equal(normalizeFocusMinutes("42.4"),42);
  assert.equal(normalizeFocusMinutes(999),600);
  assert.equal(normalizeFocusMinutes("bad",25),25);
  assert.equal(focusMinutesFromSeconds(60),1);
  assert.equal(focusMinutesFromSeconds(149),2);
});
test("到达与离开时间只累计有效停留时长",()=>{
  assert.equal(checkinDurationMinutes("2026-07-31T01:00:00.000Z","2026-07-31T02:35:00.000Z"),95);
  assert.equal(checkinDurationMinutes("2026-07-31T01:00:00.000Z","",Date.parse("2026-07-31T01:42:00.000Z")),42);
  assert.equal(checkinDurationMinutes("invalid","2026-07-31T02:00:00.000Z"),0);
  assert.equal(checkinDurationMinutes("2026-07-31T03:00:00.000Z","2026-07-31T02:00:00.000Z"),0);
});
...
test("任务可设置到达地点，并支持离开、修改、删除和月度统计",()=>{
  const source=readFileSync(fileURLToPath(new URL("../app/page.tsx",import.meta.url)),"utf8");
  const database=readFileSync(fileURLToPath(new URL("../lib/db.ts",import.meta.url)),"utf8");
  assert.match(source,/打卡地点或场景（可选）/);
  assert.match(source,/到达打卡/);
  assert.match(source,/离开打卡/);
  assert.match(source,/function CheckinInsights\(/);
  assert.match(source,/历史累计/);
  assert.match(source,/打卡记录已修改/);
  assert.match(source,/打卡记录已删除/);
